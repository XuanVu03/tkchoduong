...
function  initEasybook() {
    ...
    $(".act-hiddenpanel").on("click", function () {
        $(this).toggleClass("active-hidden-opt-btn").find("span").text($(this).find("span").text() === 'Đóng' ? 'Lựa chọn khác' : 'Đóng');
    ...
    $('.header-search-input').daterangepicker({
    ...
    $('.header-search-input').on('apply.daterangepicker', function (ev, picker) {
        $(this).val(picker.startDate.format('DD/MM/YYYY') + ' - ' + picker.endDate.format('DD/MM/YYYY'));
    });
    $('.header-search-input').on('cancel.daterangepicker', function (ev, picker) {
        $(this).val('');
    });
    ...
    $('.date-ranger').on('apply.daterangepicker', function (ev, picker) {
        $(this).val(picker.startDate.format('DD/MM/YYYY') + ' - ' + picker.endDate.format('DD/MM/YYYY'));
    });
    ...
    $('.listing-carousel').not('.slick-initialized').slick({
        infinite: true,
        slidesToShow: 5,
        dots: true,
        arrows: false,
...
function addToCartSuccess(formdata) {
    ...
    _POST_FORM(formdata, public_link('booking/checkout/addToCart'), {callback: callBack});
}

function cart_load_number() {
    var _URL = BASE_URL_ADMIN+'/data/';
    let callBack = function (res) {
        $('.wl_counter').text(res.data.number);
        if(typeof res.data.details !== 'undefined') {
            let data = res.data.details;
            $('#wishlist_otoke').empty()
            for (const i in data) {
                var html = '<li class="clearfix">\n' +
                    '                    <a href="#" class="widget-posts-img"><img src="'+_URL+data[i]['avatar']['relative_link']+'" class="respimg" alt="'+data[i]['name']+'"></a>\n' +
                    '                    <div class="widget-posts-descr">\n' +
                    '                        <a class="sp-line-2 " href="'+BASE_URL+data[i]['alias']+'.html" title="'+data[i]['name']+'">'+data[i]['name']+'</a>\n' +
                    '                        <div class="geodir-category-location fl-wrap">' +
                    '                           <a href="javascript:void(0);" class="me-2"><i class="fas fa-money-bill-alt"></i> '+formatMoney(data[i]['gia_nguoi_lon'])+'</a>' +
                    '                           <a href="javascript:void(0);" class="text-danger" onclick="delCart($(this))" data-sku="'+data[i]['sku']+'"><i class="text-danger fas fa-trash-alt"></i> Xoá</a>' +
                    '                       </div>\n' +
                    '                    </div>\n' +
                    '                </li>';
                $('#wishlist_otoke').append(html);
            }
        }

    }
    _GET_URL(public_link('booking/checkout/load'), {callback: callBack});
}
...
function delCart(dom) {
    ...
    _GET_URL(public_link('booking/checkout/cartRemove?id='+id), {callback: callBack});
}

if($('.them-vao-gio').length > 0) {
    $('.them-vao-gio').click(function () {
        addToCartSuccess('#inputCart');
    })
}
