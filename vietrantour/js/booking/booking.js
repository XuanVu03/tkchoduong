...
const HTTP = axios.create({
    baseURL: `https://vietrantour.local/`,
    headers: {
...
const regionBooking = new Vue({
    ...
    data() {
        return {
            ...
            tourSKU: tourSKU,
            tourKhoiHanhSKU: tourKhoiHanhSKU,
            tour: tour,
            current_fs: '',
            next_fs: '',
            previous_fs: '',
    ...
    watch: {
        'dytTreEm.total' : function () {
            if (this.dytTreEm.total < 0){
                return this.dytTreEm.total = 0
            }
            else {
                this.dytTreEm.total = this.dytTreEm.total
            }
        }
    },
    computed: {
    ...
    methods: {
        ...
        newMembers(key) {
            var tpl = { name: '', gender: 'male', phone: '', email: '' };
            var io = this;
            axios.post('/public_api/tour/check-member-valid', {
                    headers: {
                        Authorization: 'Bearer {_token}',
                    },
                    nguoiLon: io.dytNguoiLon,
                    treEm: io.dytTreEm,
                    treNho: io.dytTreNho,
                    tourSKU: io.tourSKU,
                    tourKhoiHanhSKU: io.tourKhoiHanhSKU,
                    typeTour: io.typeTour
                })
                .then(response => {
                    json = response.data
                    if (json.status == 1) {
        ...
        _save(key) {
            var io = this;
            io.modulesClick = true
            const api = axios.post('/booking/_save', {
                    headers: {
                        Authorization: 'Bearer {_token}',
                    },
                    member: io.member,
