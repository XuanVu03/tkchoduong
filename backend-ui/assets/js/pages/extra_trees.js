...
$(function () {
    ...
    $(".tree-json").fancytree({
        ...
        click: function (event, data) {
            ...
            if($(event.toElement).attr('class')=='fancytree-title'){
        ...
        contextMenu: {
            menu: {
                ...
                "sub": {"name": "Thêm folder con", "icon": "add"},
                "file": {"name": "Thêm file", "icon": "add"},
                "copy": {"name": "Sao chép folder", "icon": "copy"},
                "paste": {"name": "Paste", "icon": "paste"},
                "delete": {"name": "Xóa", "icon": "delete"},
                "sep1": "---------",
                "quit": {"name": "Đóng", "icon": "quit"},
            },
            actions: function (node, action, options) {
                let folderSelected = node.data;
                let parentFolderSelected = node.parent.data ? node.parent.data : {};
                let idSelected = folderSelected._id
                let token = folderSelected.token
                let type = folderSelected.type_folder

                //alert(JSON.stringify(folderSelected));
                //alert(idSelected);return;
                if (action == 'edit') {
                    return _SHOW_FORM_REMOTE(admin_link('/folder/input?id=' + idSelected)); //sửa folder
                } else if (action == 'delete') {
                    return MNG_POST.deleteItem(admin_link('/folder/_delete?id=' + idSelected+'&token='+token));
                } else if (action == 'copy') {
                    FOLDER_COPIED = folderSelected;
                    alert('Đã copy folder '+folderSelected.name);
                } else if (action == 'paste') {
                    if(!FOLDER_COPIED){
                        alert('Bạn cần chọn folder rồi copy trước');
                    }else{
                        if(FOLDER_COPIED._id==idSelected){
                            alert('Case copy A và paste vào A');
                            return false;
                        }
                        if(!confirm('Folder "'+FOLDER_COPIED.name+'" sẽ được copy vào trong folder "'+folderSelected.name+'"?')){
                            return false;
                        }
                        var callBack = function (json) {
                            FOLDER_COPIED = false;//reset lại
                            alert(json.msg);
                        };
                        return _POST(admin_link('/folder/do-paste?from_id=' + FOLDER_COPIED._id+'&token='+token+'&to_id='+idSelected), [], callBack);
                    }
                } else if (action == 'sub') {
                    //Thêm folder con=> bật form thêm lên và mặc định selected cha là ông này luôn
                    return _SHOW_FORM_REMOTE(admin_link('/folder/input?parent_id=' + idSelected+'&type_folder='+type));
                }
                else if (action == 'file') {
                    //nếu là folder cha thì ko thêm được file
                    if(node.children !== null){
                        alert("Không thể thêm file vào folder cha, chỉ được phép thêm vào folder con");
                    }
                    else{
                        //Thêm file vào folder, chỉ thêm được vào cấp folder con cuối cùng
                        return _SHOW_FORM_REMOTE(admin_link('/profile/show_popup?folder_id=' + idSelected+'&file_type='+type));
                    }
                }
            }
        },
